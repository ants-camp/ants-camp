...
/**
 * 랭킹 서비스가 수신하는 Kafka 토픽명을 관리한다. - topics.trade.*  : 매매 서비스(trade-service)로부터 수신 - topics.asset.*  : 자산
 * 서비스(asset-service)로부터 수신
 */
@Validated
@ConfigurationProperties(prefix = "topics")
public record RankingTopicProperties(
        @Valid @NotBlank Trade trade,
        @Valid @NotBlank Asset asset
) {
    public record Trade(
